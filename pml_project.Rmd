...
# Set QUICK_MODE = TRUE for faster execution (uses sample data and fewer trees)
# Set QUICK_MODE = FALSE for full analysis (recommended for final submission)
QUICK_MODE <- FALSE
```
...
# For quick mode, use a sample of the data
if (QUICK_MODE) {
  cat("QUICK MODE: Using sample of 3000 rows\n")
  training_sample <- training_clean[sample(nrow(training_clean), 3000), ]
...
if (QUICK_MODE) {
  cat("QUICK MODE: Using 3-fold CV and 50 trees\n")
  ctrl <- trainControl(method = "cv", number = 3, verboseIter = FALSE)
  n_trees <- 50
} else {
  cat("FULL MODE: Using 5-fold CV and 100 trees\n")
